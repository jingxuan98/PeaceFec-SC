...
contract PFSPLoanWallet {
    ...
    function createTransfer(uint256 amount) external onlyApprover {
        require(amount >= 0.001 ether, "Not enough funds in the contract yet");
        require(amount <= address(this).balance, "Not enough funds in the contract yet");
    ...
    function transferFund(uint256 id) internal {
        ...
        uint256 amount1 = transfers[id].amount * (approver1Percentage / denominator);
        uint256 amount2 = transfers[id].amount * (approver2Percentage / denominator);
    ...
    function setApproverPercentage(uint256 _approver1Percentage, uint256 _approver2Percentage)
        external
        onlyAdmin
    {
